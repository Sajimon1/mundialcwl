...
import { PlusCircle, Edit3, Settings, Save, Sparkles, RefreshCw, Trash2, Calendar, Users, UserX } from 'lucide-react';
...
export default function AdminPanel({
  ...
  return (
    <div className="bg-slate-900/50 border border-slate-800/80 rounded-[2.25rem] p-6 backdrop-blur-md shadow-[0_0_30px_rgba(217,119,6,0.05)] relative overflow-hidden" id="admin-panel">
      ...
      {/* Admin Tabs */}
      <div className="flex flex-col sm:flex-row p-1 gap-1.5 mb-6 bg-slate-950/60 rounded-2xl border border-slate-850">
        <button
        ...
        >
          ...
          Zarządzaj graczami ({users.length})
        </button>
      </div>
      ...
      {/* Tab: Manage Users / Remove profile entirely */}
      {activeTab === "users" && (
        <div className="space-y-4 max-h-[55vh] overflow-y-auto pr-1 z-10 relative">
          <div className="p-3.5 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-[11px] text-amber-300 leading-relaxed font-sans mb-2">
            <p className="font-extrabold text-amber-400 text-[10px] uppercase tracking-wider font-mono mb-1">🚨 Panel Usuwania Profili / Graczy</p>
            Jako administrator możesz całkowicie usunąć błędnie/przypadkowo utworzone profile z bazy danych. Usunięcie profilu wykasuje również wszystkie jego typy, a tabela zostanie automatycznie przeliczona.
          </div>
          ...
          {users.length === 0 ? (
            ...
            users.map(user => (
              <div key={user.uid} className="p-4 bg-slate-950/40 rounded-2xl border border-slate-855 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  <img src={user.avatarUrl} alt={user.username} className="w-9 h-9 rounded-full border border-slate-700 shrink-0" referrerPolicy="no-referrer" />
                  <div className="min-w-0">
                    <h4 className="text-sm font-extrabold text-slate-100 flex items-center gap-2 truncate">
                      <span>{user.username}</span>
                      {user.isAdmin && (
                        <span className="text-[8px] bg-amber-550/20 border border-amber-550/30 text-amber-400 px-1.5 py-0.5 rounded font-mono font-black uppercase">ADMIN</span>
                      )}
                    </h4>
                    <p className="text-[10px] text-slate-500 font-mono">
                      ID: <span className="text-slate-400">{user.uid}</span> • Punkty: <span className="text-indigo-400 font-bold">{user.points} pkt</span>
                    </p>
                  </div>
                </div>
                  ...
                  <button
                    onClick={async () => {
                      const confirmDel = await customConfirm(
                        `Czy na pewno chcesz CAŁKOWICIE usunąć profil gracza "${user.username}" z bazy danych w chmurze? \n\nZostaną usunięte wszystkie jego typowania i statystyki! Ta operacja jest nieodwracalna.`,
                        "Usunięcie gracza"
                      );
                      if (confirmDel) {
                        try {
                          await onDeleteUser(user.uid);
                          await customAlert(`Profil "${user.username}" i wszystkie jego typowania zostały pomyślnie usunięte!`, "Skasowano pomyślnie");
                        } catch (err: any) {
                          console.error("Failed to delete user profile", err);
                          await customAlert(`Nie udało się skasować profilu: ${err.message || err}`, "Błąd");
                        }
                      }
                    }}
                    className="flex items-center justify-center bg-rose-950/20 hover:bg-rose-600 border border-rose-950/45 hover:border-rose-500 text-rose-500 hover:text-white px-3 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition cursor-pointer gap-1.5"
                    title="Usuń trwale gracza z bazy"
                  >
                    <UserX className="w-3.5 h-3.5" />
                    <span>Skasuj gracza</span>
                  </button>
                </div>
              </div>
            ))
